...
function detectLanguage(tags: string[], courseSlug: string): string {
  ...
  if (tags.some((t: string) => t.toLowerCase().includes("python")))
    return "python";
  if (tags.some((t: string) => t.toLowerCase().includes("golang"))) return "go";
  ...
  // 4. Last resort: check solution code patterns
  return "unknown";
}
