...
function detectLanguage(tags: string[]): string {
  ...
  if (tags.some((t) => t.includes('py') || t.includes('ml'))) return 'python';
  if (tags.some((t) => t.includes('go'))) return 'go';
  if (tags.some((t) => t.includes('java'))) return 'java';
  if (tags.some((t) => t.includes('js') || t.includes('node'))) return 'javascript';
